...
The project is a Node.js serverless app deployed on Vercel. The public surface is a single endpoint `POST /api/chat`. It returns JSON `{ text }` (the client owns the conversation history). LLM access uses Google Gemini (free AI Studio tier) over OpenAI-compatible chat-completions (`lib/llm/client.js`).
...
api/chat.js              # POST /api/chat — intent router → fast path or tool loop → sanitized output (imports lib/http/*)
lib/http/                # cors.js, rate-limit.js, validate.js — extracted from api/chat.js for testability
lib/chatbot/
  router/                # bilingual (EN/VI) intent router: knowledge-only vs live-data (+ tiny LLM fallback)
  quick-answers/         # deterministic no-LLM answers (center info, Vipassana definition, FAQ)
  schedule/              # deterministic schedule answers (windowed + default-upcoming queries, no LLM)
  out-of-scope.js        # deterministic out-of-scope fallback
lib/knowledge/
  loader.js              # loads SKILL.md from disk, module-cached
  parser.js              # splits SKILL.md into numbered sections
  sections.js            # SKILL.md sectioning + fast-path prompt builder (trimmed knowledge context)
  system-prompt.js       # KNOWLEDGE_SYSTEM_PROMPT template ({knowledge_base} placeholder)
  retrieval/             # tokenize.js, kb.js (BM25), center-intent.js + index.js
lib/i18n/                # lang.js (Lang type + detectLanguage), normalize.js (stripDiacritics)
lib/llm/                 # provider.js (registry, GEMINI_API_KEY), client.js (chatCompletion, 429 backoff)
lib/security/            # sanitize.js — TRUSTED_DOMAINS + sanitize_urls() backstop
lib/observability/       # log.js — structured JSON-line logging (AsyncLocalStorage, safeErr, qPreview/qHash)
lib/cache/               # answer-cache.js — in-memory TTL cache for fast-path answers
lib/centers/             # centers.js (static center info), fallback-schedule.json (static fallback)
lib/scraper/             # vri-schedule.js, ucenlist-schedule.js, schedule-cache.js (TTL + fallback), list-courses.js (orchestrator)
lib/config/              # constants.js — shared timeouts/limits
lib/*.js                 # barrels re-exporting new locations for backward compat (router.js, quick-answers.js, etc.)
lib/fallback-schedule.json # duplicate of lib/centers/fallback-schedule.json for test compat
...
Only `api/chat.js` lives under `api/` — everything else moved to `lib/` (Vercel's 12-function-per-deployment limit counts every `api/*.js` file as a serverless function). `find api -name '*.js'` must return exactly `api/chat.js`. Subdirectories under `lib/` do NOT count as functions.

### Request flow (relevant to latency work)

`api/chat.js` classifies the latest user message with `lib/chatbot/router` (`lib/chatbot/router/index.js`):

- **Knowledge-only (`kb`)** → fast path, in this order: (1) deterministic structured answers via `lib/chatbot/quick-answers` (center address/phone/email/website from `lib/centers/centers.js`, curated bilingual Vipassana definition, and common FAQs like cost/diet/eligibility) with **no LLM call**; (2) the in-memory answer cache (`lib/cache/answer-cache.js`, keyed by `lang|normalized question`) for repeated questions; (3) a single LLM call with a trimmed system prompt (only the relevant SKILL.md sections via `lib/knowledge/sections.js`), **no tools attached**. Every fast-path output passes through `sanitize_urls()` (`lib/security/sanitize.js`). Every LLM call (classifier, fast path, composer, retries) goes through `lib/llm/client.js` + `provider.js`: Gemini (`gemini-3.1-flash-lite-preview`, `GEMINI_API_KEY`) with 429 backoff, and the model resolved as `AGENT_MODEL` > provider default.
- **Live-data (`tools`)** → first the deterministic schedule fast path (`lib/chatbot/schedule/index.js`): a windowed schedule query (a schedule/course keyword plus a center and/or time window such as "cuối tháng này", "tháng này", "tháng sau", "tuần này/khác", "tháng N") calls `list_courses` (`lib/scraper/list-courses.js`) once and renders a bilingual markdown answer with **no LLM call and no tools**. A bare schedule query with a course noun but no center/time window ("khóa thiền sắp tới", "which courses") defaults to the upcoming-courses list across both centers. Anything that doesn't match (or fails) returns `null` and falls through to the composer path. `list_courses` scrapes `schedule.vridhamma.org` (parallel across centers for `center="all"`, in-flight dedup) with a live → cached → fallback chain; every record carries `data_freshness` (`"live"` / `"cached"` / `"fallback"`).
...
1. **Safe Domain Gating**: only `ucenlist.org` and `*.vridhamma.org` URLs may ever reach the user — enforced both in the system prompt and programmatically via `sanitize_urls()`/`TRUSTED_DOMAINS` in `lib/security/sanitize.js` (re-exported via `lib/sanitize.js` for compat). This dual-layer ("prompt says don't + code enforces it") is intentional — treat both layers as required when touching domain-gating logic. The fast path still passes through `sanitize_urls()`.
