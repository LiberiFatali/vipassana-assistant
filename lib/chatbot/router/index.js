 ...
 */
import { classifyByRetrieval } from "../../knowledge/retrieval/kb.js";
import { chatCompletion } from "../../llm/client.js";
import { detectLanguage, normalize } from "../../i18n/lang.js";

// Re-export Lang primitives for backward compatibility (tests still import from router)
export { detectLanguage, isLang, LANG_EN, LANG_VI, LANGS, normalize } from "../../i18n/lang.js";

/**
 * @typedef {import("../../i18n/lang.js").Lang} Lang
 * @typedef {{kind:"kb"|"tools"|"ambiguous", lang:Lang}} RouteDecision
 */
...
/**
 * @param {unknown} text
 * @returns {Promise<{kind:"kb"|"tools", lang:Lang}>}
 */
export async function classifyIntent(text) {
  const local = classifyLocal(text);
  if (local.kind !== "ambiguous") {
    return /** @type {{kind:"kb"|"tools", lang:Lang}} */ (local);
  }

  const retrieval = classifyByRetrieval(/** @type {string} */ (text));
  if (retrieval.kind !== "ambiguous") {
    return { kind: /** @type {"kb"|"tools"} */ (retrieval.kind), lang: local.lang };
  }

  const kind = await classifyWithLLM(/** @type {string} */ (String(text || "")));
