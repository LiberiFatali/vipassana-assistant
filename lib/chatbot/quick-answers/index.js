...
import { normalize } from "../../i18n/lang.js";
import { detectCenterInfoIntent } from "../../knowledge/retrieval/center-intent.js";
...
export const CENTER_CUES = [
  ...
  { key: "pala", keywords: ["dhamma pala", "pala", "bodh gaya", "bodhgaya", "ấn độ", "an do", "india"] },
];
...
function formatCenter(center, lang) {
  ...
  return {
    ...
    email: c.email,
    website: c.website,
    schedule: lang === LANG_VI ? c.schedule_url_vi : c.schedule_url_en,
  };
}
...
function renderCenterInfo(centers, intent, lang) {
  ...
  for (const key of centers) {
    ...
    if (intent === "address") {
      push(lang === LANG_VI ? "Địa chỉ" : "Address", info.address);
    } else if (intent === "phone") {
      push(lang === LANG_VI ? "Điện thoại" : "Phone", info.phone);
    } else if (intent === "email") {
      push("Email", info.email);
    } else if (intent === "website") {
      push("Website", info.website);
    ...
    push(lang === LANG_VI ? "Lịch khóa" : "Course schedule", info.schedule);
    lines.push("");
  }
  return lines.join("\n").trim();
}
 ...
 * @param {Lang} lang detected language
 * @returns {string|null}
 */
export function getQuickAnswer(text, lang) {
  const n = normalize(text);
  const detectedLang = lang === LANG_VI ? LANG_VI : "en";
  ...
  if (
    ...
    return detectedLang === LANG_VI ? VIPASSANA_DEF_VI : VIPASSANA_DEF_EN;
  }
  ...
  if (faq) {
    return detectedLang === LANG_VI ? faq.vi : faq.en;
  }
