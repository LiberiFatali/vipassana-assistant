 ...
 * LLM tool loop.
 */
import { CENTERS } from "../../centers/centers.js";
import { LANG_VI } from "../../i18n/lang.js";
import { detectLanguage, normalize } from "../../i18n/lang.js";
import { BOTH_CENTERS_CUES, CENTER_CUES } from "../quick-answers/index.js";
import { VRI_SCHEDULE_URLS } from "../../scraper/vri-schedule.js";
import { listCourses as listCoursesDefault } from "../../scraper/list-courses.js";
import { logWarn, safeErr } from "../../observability/log.js";

/** @typedef {import("../../i18n/lang.js").Lang} Lang */
/**
 * @typedef {import("../../scraper/list-courses.js").Course} Course
 * @typedef {{lang:Lang, centers:Set<string>, both:boolean, window:{from:string,to:string}|null, timeId:string|null, remind:boolean, now:Date, n:string}} ScheduleIntent
 */

// Center keys in canonical display order (virocana, vutthi, pala). Derived so
// adding a center to CENTERS automatically extends the default/upcoming answer.
const ALL_CENTER_KEYS = Object.keys(CENTERS);
// Real VRI centers — the ones whose schedule is scraped from schedule.vridhamma.org.
const VRI_CENTER_KEYS = Object.keys(VRI_SCHEDULE_URLS);

// A course is a special announcement when its record is marked type "special"
// (set by enrichPalaCourse and by the static fallback JSON). Announcements are
// dateless and render by title; detection is by record marker, not center id,
// so announcements from any derived center_id (e.g. the scraper's "special"
// fallback or a future slug) are handled the same way.
/**
 * @param {Course} course
 * @returns {boolean}
 */
function isAnnouncement(course) {
  return course.type === "special";
}
...
function selectedCenterKeys(query) {
  ...
  // Default/both-center queries also surface UCENLIST special announcements
  // (the non-VRI pseudo-centers) alongside the two VRI centers.
  if (both || centers.size === 0) {
    return ALL_CENTER_KEYS;
  }
  if (centers.size === 2) {
    return VRI_CENTER_KEYS;
  }
  return [...centers];
}
...
function statusText(status, lang) {
  const map = {
    open: lang === LANG_VI ? "đang nhận đăng ký" : "open for registration",
    full: lang === LANG_VI ? "đã hết chỗ" : "full",
    waitlist: lang === LANG_VI ? "danh sách chờ" : "waitlist",
    unknown: lang === LANG_VI ? "trạng thái chưa rõ" : "status unknown",
  };
...
function centerHeading(centerId, lang, sampleCourse) {
  const c = CENTERS[centerId];
  if (!c) {
    // Unknown announcement bucket (e.g. the scraper's "special" fallback) —
    // render with the record's center name, no city, no crash.
    return `**${(sampleCourse && sampleCourse.center) || centerId}**`;
  }
  const city = lang === LANG_VI ? c.city_vi : c.city;
  return `**${c.name}** — ${city}`;
}
...
function scheduleUrl(centerId, lang) {
  ...
  return lang === LANG_VI ? c.schedule_url_vi : c.schedule_url_en;
}
...
function applyLink(course, lang) {
  ...
  return `[${lang === LANG_VI ? "Đăng ký" : "Apply"}](${course.apply_url})`;
}

function fallbackWarning(query) {
  if (query.lang === LANG_VI) {
...
export function formatScheduleAnswer(query, courses) {
  ...
  const inWindow = (c) => {
    if (!c.start_date || !ISO_DATE_RE.test(c.start_date)) {
      // Dateless UCENLIST announcements only appear in the no-window
      // "upcoming" listing — never in dated time-window queries.
      return query.window ? false : isAnnouncement(c);
    }
    if (query.window) {
  ...
  const filtered = courses
    .filter((c) => (isAnnouncement(c) || selected.includes(c.center_id)) && inWindow(c))
  ...
  // Render order: selected centers first, then any announcement buckets whose
  // derived center_id is not a known CENTERS key (e.g. "special"), so they
  // still render instead of being silently dropped.
  const renderOrder = [...selected];
  for (const centerId of byCenter.keys()) {
    if (!renderOrder.includes(centerId)) {
      renderOrder.push(centerId);
    }
  }
  ...
  let listed = 0;
  const MAX_LISTED = 8;
  for (const centerId of renderOrder) {
    const coursesOf = byCenter.get(centerId) || [];
    if (coursesOf.length === 0) {
      continue;
    }
    lines.push(centerHeading(centerId, query.lang, coursesOf[0]));
    for (const course of coursesOf) {
      // UCENLIST announcements always render, regardless of the dated-course
      // cap — they are few and the most current.
      if (!isAnnouncement(course) && listed >= MAX_LISTED) {
      ...
      const dateless = !course.start_date || !ISO_DATE_RE.test(course.start_date);
      let line;
      if (dateless && course.title) {
        // Announcement (no dates): render the title instead of a date range.
        line = `- **${course.title}**: ${statusText(course.status, query.lang)}`;
      } else {
        ...
        line = `- **${typeLabel(course.type, query.lang)}**: ${dates} — ${statusText(course.status, query.lang)}`;
      }
      // Announcements carry no apply link — the "Full schedule:" footer below
      // links to the official UCENLIST course-schedule page for them.
      if (!dateless) {
  ...
  lines.push(query.lang === LANG_VI ? "Xem lịch đầy đủ:" : "Full schedule:");
  for (const centerId of selected) {
    const url = scheduleUrl(centerId, query.lang);
    if (url) {
      lines.push(`- ${url}`);
    }
  }
 ...
 *
 * @param {string} text  the user's latest message
 * @param {Lang} [lang] detected language override (detected if omitted)
...
export async function getScheduleAnswer(text, lang, { list = listCoursesDefault, now = new Date() } = {}) {
  try {
    ...
    if (lang) {
      detected.lang = lang === LANG_VI ? LANG_VI : "en";
    }
    ...
    logWarn("schedule.deterministic.error", { ...safeErr(err) });
    return null;
  }
}
 ...
 * @param {unknown} text
 * @param {Lang} [lang]
 * @param {{list?: (arg:any)=>Promise<Course[]>}} [opts]
 * @returns {Promise<string>}
 */
export async function buildLiveScheduleContext(text, lang = LANG_VI, { list = listCoursesDefault } = {}) {
  try {
    const courses = await list({ center: "all", language: /** @type {Lang} */ (lang) });
      ...
      .map((c) => {
        const when =
          c.title && !c.start_date
            ? `Title: ${c.title}`
            : `Dates: ${formatHumanDate(c.start_date)} - ${formatHumanDate(c.end_date)}`;
        return `- Center: ${c.center} (${c.center_id}) | Type: ${c.type} | ${when} | Status: ${c.status} | Apply: ${c.apply_url || "N/A"}`;
      })
      .join("\n");
    return `Live Course Schedule Context (use this data to compose your response):\n${formatted}`;
  } catch (err) {
    logWarn("schedule.context.error", { ...safeErr(err) });
