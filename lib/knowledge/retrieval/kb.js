...
/**
 * @typedef {{kind:"kb"|"tools"|"ambiguous", kbScore:number, toolsScore:number}} RetrievalClassification
 * @typedef {{id:string, score:number, cls:"kb"|"tools"}} RetrievalResult
 */
 ...
 * @returns {{num:number, vi:boolean}}
 */
function parseDocId(id) {
  const vi = typeof id === "string" && id.endsWith("-VI");
  const num = vi ? Number(String(id).slice(0, -3)) : Number(id);
  return { num, vi };
}
 ...
 * @param {unknown} text
 * @returns {RetrievalClassification}
 */
...
// Re-export center-info intent detection for backward compatibility — canonical
// implementation lives in ./center-intent.js
export {
  CENTER_INFO_INTENT_FLOOR,
  CENTER_INFO_INTENT_LABELS,
  detectCenterInfoIntent,
} from "./center-intent.js";
