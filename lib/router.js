...
// Conservative classifier budget. The tiny max_tokens is intentional: free
// reasoning models (e.g. `mimo-v2.5-free`, `deepseek-v4-flash-free`) consume
// the token budget on `reasoning_content` before any content, so a meaningful
// classifier answer is not reliably achievable within a short timeout. We keep
// the 2.5s deadline and let a timeout fall back to the tool path (conservative:
// extra latency, never wrong data). The tool path succeeds with the fast
// default model, so a misrouted knowledge question still gets a real answer.
...
const KB_STRONG = [
  ...
  "bản đồ", "ban do", "đường đi", "duong di",
...
/**
 * Resolve an ambiguous request with a tiny LLM call. Returns "tools" on any
 * failure/timeout (conservative: extra latency, never wrong data).
 */
async function classifyWithLLM(text, apiKey, modelId) {
  try {
 ...
 */
export async function classifyIntent(text, apiKey, modelId) {
  ...
  const kind = await classifyWithLLM(text, apiKey, modelId);
